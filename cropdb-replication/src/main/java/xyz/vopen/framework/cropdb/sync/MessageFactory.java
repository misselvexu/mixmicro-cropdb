...
import xyz.vopen.framework.cropdb.sync.message.*;
...
public class MessageFactory {
  ...
  public BatchChangeContinue createChangeContinue(
      Config config, String replicaId, String uuid, LastWriteWinState state) {
    BatchChangeContinue message = new BatchChangeContinue();
    message.setHeader(
        createHeader(
            MessageType.BatchChangeContinue,
            config.getCollection().getName(),
            uuid,
            replicaId,
            config.getUserName()));
  ...
  public BatchChangeEnd createChangeEnd(
      Config config, String replicaId, String uuid, Long lastSyncTime) {
    BatchChangeEnd message = new BatchChangeEnd();
    message.setHeader(
        createHeader(
            MessageType.BatchChangeEnd,
            config.getCollection().getName(),
            uuid,
            replicaId,
            config.getUserName()));
  ...
  public DataGateFeedAck createFeedAck(
      Config config, String replicaId, String correlationId, Receipt receipt) {
    DataGateFeedAck ack = new DataGateFeedAck();
    ack.setHeader(
        createHeader(
            MessageType.DataGateFeedAck,
            config.getCollection().getName(),
            correlationId,
            replicaId,
            config.getUserName()));
  ...
  public BatchAck createBatchAck(
      Config config, String replicaId, String correlationId, Receipt receipt) {
  ...
  public MessageHeader createHeader(
