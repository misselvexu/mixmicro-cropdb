...
import xyz.vopen.framework.cropdb.sync.handlers.*;
import xyz.vopen.framework.cropdb.sync.message.DataGateMessage;
import xyz.vopen.framework.cropdb.sync.net.DataGateSocketListener;
              ...
              () -> {
                try {
                  ...
                  log.error(
                      ...
                      error);
                  if (error.isFatal()) {
                    replicationTemplate.postEvent(
                        new ReplicationEvent(ReplicationEventType.Error, error));
                  ...
                  log.error(
                      ...
                      e);
                  replicationTemplate.postEvent(
                      new ReplicationEvent(ReplicationEventType.Error, e));
                  replicationTemplate.stopReplication("Error - " + e.getMessage());
                }
              });
    }
  ...
  private void validateMessage(DataGateMessage message) {
    if (message == null) {
      throw new ReplicationException(
          "a null message is received for " + replicationTemplate.getReplicaId(), true);
    } else if (message.getHeader() == null) {
      throw new ReplicationException(
          "a message without header is received for " + replicationTemplate.getReplicaId(), true);
    } else if (StringUtils.isNullOrEmpty(message.getHeader().getCollection())) {
      throw new ReplicationException(
          "a message without collection info is received for " + replicationTemplate.getReplicaId(),
          true);
    } else if (message.getHeader().getMessageType() == null) {
      throw new ReplicationException(
          "a message without any type is received for " + replicationTemplate.getReplicaId(), true);
    }
  }
