...
public class MigrationManager {
        ...
        if (isMigrationNeeded()) {
            ...
            if (migrationPath == null || migrationPath.isEmpty()) {
                ...
                throw new MigrationException("schema version mismatch, as no migration path found from version "
                    + storeMetadata.getSchemaVersion() + " to " + cropConfig.getSchemaVersion());
            }
    ...
    private void executeMigrationSteps(Queue<MigrationStep> migrationSteps) {
        ...
        CropMap<String, Document> storeInfo = database.getStore().openMap(STORE_INFO,
        ...
        if (step != null) {
            ...
            switch (step.getInstructionType()) {
                case AddPassword:
                    ...
                    command = crop -> {
                        UserAuthenticationService authService = new UserAuthenticationService(crop.getStore());
