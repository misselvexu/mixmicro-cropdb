...
public class UpdateOptions {
  ...
  /**
   * Indicates if the update operation will insert a new document if it does not find any existing
   * document to update.
   *
   * @param insertIfAbsent a value indicating, if a new document to insert in case the filter fails
   *     to find a document to update.
   * @return `true` if a new document to insert; otherwise, `false`.
   * @see CropCollection#update(Filter, Document, UpdateOptions)
   */
  @Getter @Setter private boolean insertIfAbsent;
   ...
   */
  @Getter @Setter private boolean justOnce;
