...
/**
 * Represents a type-safe persistent java object collection. An object repository is backed by a
 * {@link CropCollection}, where all objects are converted into a {@link Document} and saved into
 * the database.
 *
 * <p>An object repository is observable like its underlying {@link CropCollection}. <b>Create a
 * repository</b>
 *
 * <pre>{@code
...
public interface ObjectRepository<T> extends PersistentCollection<T> {
  /**
   * Inserts object into this repository. If the object contains a value marked with {@link Id},
   * then the value will be used as a unique key to identify the object in the repository. If the
   * object does not contain any value marked with {@link Id}, then crop will generate a new {@link
   * CropId} and will add it to the document generated from the object.
   *
   * <p>If any of the value is already indexed in the repository, then after insertion the index
   ...
   * @throws InvalidIdException if the id value contains non comparable type, i.e. type that does
   *     not implement {@link Comparable}.
   * @throws InvalidIdException if the id contains value which is not of the same java type as of
   *     other objects' id in the collection.
   * @throws UniqueConstraintException if the value of id value clashes with the id of another
   *     object in the collection.
   * @throws UniqueConstraintException if a value of the object is indexed, and it violates the
  ...
  /**
   * Updates object in the repository. If the filter does not find any object in the collection,
   * then the {@code update} object will be inserted.
   *
   * <p>If the {@code filter} is {@code null}, it will update all objects in the collection.
   *
   * <p><b>CAUTION:</b> If the {@code update} object has a non {@code null} value in the id value,
   * this value will be removed before update.
   *
   * <p>NOTE: This operations will notify all {@link CollectionEventListener} instances registered
  ...
  /**
   * Updates object in the repository. Update operation can be customized with the help of {@code
   * updateOptions}.
   *
   * <p>If the {@code filter} is {@code null}, it will update all objects in the collection unless
   ...
   * @param filter the filter to apply to select objects from the collection.
   * @param update the modifications to apply.
   * @param insertIfAbsent if set to {@code true}, {@code update} object will be inserted if not
   *     found.
   * @return the result of the update operation.
   * @throws ValidationException if the {@code update} object is {@code null}.
   ...
   * <p>If the {@code filter} is {@code null}, it will update all objects in the collection.
   *
   * <p><b>CAUTION:</b> The {@code update} document should not contain {@code _id} field.
   *
   * <p>NOTE: This operations will notify all {@link CollectionEventListener} instances registered
   * to this collection with change type {@link EventType#Update}.
   *
  ...
  /**
   * Updates object in the repository by setting the field specified in {@code document}. Update
   * operation can either update the first matching object or all matching objects depending on the
   * value of {@code justOnce}.
   *
   * <p>If the {@code filter} is {@code null}, it will update all objects in the collection unless
   ...
   *
   * <p>If the {@code filter} is {@code null}, it will remove all objects from the collection.
   *
   * <p>NOTE: This operations will notify all {@link CollectionEventListener} instances registered
  ...
  /**
   * Removes object from the collection. Remove operation can be customized by {@code
   * removeOptions}.
   *
   * <p>If the {@code filter} is {@code null}, it will remove all objects in the collection unless
   ...
   *
   * <p>See {@link Filter} for all available filters.
   *
   * <p>NOTE: If there is an index on the value specified in the filter, this operation will take
  ...
  /**
   * Gets a single element from the repository by its id. If no element is found, it will return
   * {@code null}. The object must have a field annotated with {@link Id}, otherwise this call will
   * throw {@link InvalidIdException}.
   *
