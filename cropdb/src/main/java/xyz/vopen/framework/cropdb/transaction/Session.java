...
/**
 * A crop transaction session. A session is needed to initiate a transaction in crop database.
 *
 * <p>If a session is closed and the transaction is not committed, all opened transactions will get
 * rolled back and all volatile data gets discarded for the session.
 *
...
public class Session implements AutoCloseable {
  ...
  /**
   * Checks state of the session. If the session is not active, it will throw a {@link
   * TransactionException}.
   *
   * @throws TransactionException when the session is not active, and a transaction is initiated in
   *     this session.
   */
