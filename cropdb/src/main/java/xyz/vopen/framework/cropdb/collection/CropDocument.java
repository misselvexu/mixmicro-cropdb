...
class CropDocument extends LinkedHashMap<String, Object> implements Document {
  ...
  private static final List<String> reservedFields =
      Iterables.listOf(
          Constants.DOC_ID, Constants.DOC_REVISION, Constants.DOC_SOURCE, Constants.DOC_MODIFIED);
  ...
  public Document put(String field, Object value) {
    ...
    if (value != null && !Serializable.class.isAssignableFrom(value.getClass())) {
      throw new ValidationException(
          "type " + value.getClass().getName() + " does not implement java.io.Serializable");
    }
  ...
  public Object get(String field) {
    if (field != null && isEmbedded(field) && !containsKey(field)) {
  ...
  public boolean equals(Object other) {
    ...
    if (!(other instanceof CropDocument)) return false;
    ...
    for (Pair<String, Object> entry : this) {
      ...
      if (value instanceof CropDocument) {
        ...
        if (StringUtils.isNullOrEmpty(prefix)) {
          ...
          fields.addAll(
              ((CropDocument) value)
                  .getFieldsInternal(prefix + CropConfig.getFieldSeparator() + entry.getFirst()));
        }
  ...
  private Object recursiveGet(Object object, String[] remainingPath) {
    ...
    if (object instanceof Document) {
      ...
      return recursiveGet(
          ((Document) object).get(remainingPath[0]),
    ...
    if (object.getClass().isArray()) {
      ...
      if (isInteger(accessor)) {
        ...
        if (index < 0) {
          throw new ValidationException(
              "invalid array index " + index + " to access item inside a document");
        }

        // check index upper bound
        if (index >= array.length) {
          throw new ValidationException(
              "index " + index + " is not less than the size of the array " + array.length);
        }

        // get the value at the index from the array
        // if there are remaining keys, scan to the next level
        return recursiveGet(
            array[index], Arrays.copyOfRange(remainingPath, 1, remainingPath.length));
      } else {
    ...
    if (object instanceof Iterable) {
      ...
      if (isInteger(accessor)) {
        ...
        if (index < 0) {
          throw new ValidationException(
              "invalid collection index " + index + " to access item inside a document");
        }

        // check index upper bound
        if (index >= collection.size()) {
          throw new ValidationException(
              "index " + accessor + " is not less than the size of the list " + collection.size());
        }

        // get the value at the index from the list
        // if there are remaining keys, scan to the next level
        return recursiveGet(
            collection.get(index), Arrays.copyOfRange(remainingPath, 1, remainingPath.length));
      } else {
        // if the current key is not an integer, then decompose the
