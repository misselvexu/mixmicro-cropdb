...
public class Numbers {
   ...
   * @return `0` if `x` and `y` are numerically equal. A value less than `0` if `x` is numerically
   *     less than `y`. A value greater than `0` if `x` is numerically greater than `y`.
   */
  ...
  public static Object castNumber(Object value, Class<?> type) {
    ...
    throw new ValidationException(
        "cannot cast number of type " + value.getClass().getName() + " to " + type.getName());
  }
