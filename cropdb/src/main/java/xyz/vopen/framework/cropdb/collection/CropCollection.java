...
/**
 * Represents a named document collection stored in crop database. It persists documents into the
 * database. Each document is associated with a unique {@link CropId} in a collection.
 *
 * <p>A crop collection supports indexing. Every crop collection is also observable. <b>Create a
 * collection</b>
 *
 * <pre>{@code
...
public interface CropCollection extends PersistentCollection<Document> {
  /**
   * Insert documents into a collection. If the document contains a {@code _id} value, then the
   * value will be used as a unique key to identify the document in the collection. If the document
   * does not contain any {@code _id} value, then crop will generate a new {@link CropId} and will
   * add it to the document.
   *
   * <p>If any of the value is already indexed in the collection, then after insertion the index
   * will also be updated.
   *
   * <p>NOTE: These operations will notify all {@link CollectionEventListener} instances registered
   ...
   * @throws InvalidIdException if the {@code _id} value contains non comparable type, i.e. type
   *     that does not implement {@link Comparable}.
   ...
   * @throws UniqueConstraintException if the value of {@code _id} value clashes with the id of
   *     another document in the collection.
   * @throws UniqueConstraintException if a value of the document is indexed and it violates the
   ...
   *
   * <p>If the {@code filter} is {@code null}, it will update all documents in the collection.
   *
  ...
  /**
   * Updates document in the collection. Update operation can be customized with the help of {@code
   * updateOptions}.
   *
   * <p>If the {@code filter} is {@code null}, it will update all documents in the collection unless
  ...
  /**
   * Removes document from a collection. Remove operation can be customized by {@code
   * removeOptions}.
   *
   * <p>If the {@code filter} is {@code null}, it will remove all documents in the collection unless
  ...
  /**
   * Gets a single element from the collection by its id. If no element is found, it will return
   * {@code null}.
   *
