...
public class BoundedDocumentStream implements RecordStream<Pair<CropId, Document>> {
   ...
   */
  public BoundedDocumentStream(
      Long skip, Long limit, RecordStream<Pair<CropId, Document>> recordStream) {
    this.skip = skip;
    this.limit = limit;
  ...
  private static class BoundedIterator<T> implements Iterator<T> {
    ...
    public BoundedIterator(
        final Iterator<? extends T> iterator, final long skip, final long limit) {
      if (iterator == null) {
