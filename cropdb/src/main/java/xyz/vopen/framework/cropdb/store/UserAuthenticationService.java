...
public class UserAuthenticationService {
    ...
    public void authenticate(String username, String password, boolean existing) {
        if (!StringUtils.isNullOrEmpty(password) && !StringUtils.isNullOrEmpty(username)) {
            if (!existing) {
                ...
                CropMap<String, UserCredential> userMap = store.openMap(USER_MAP, String.class, UserCredential.class);
    ...
    private byte[] hash(char[] password, byte[] salt) {
        ...
        try {
            ...
            throw new CropSecurityException("error while hashing a password: "
                + e.getMessage());
        } finally {
            spec.clearPassword();
        }
