...
/**
 * The type Crop data type.
 *
 * <p><b>NOTE:</b> This code is a modification of h2 mvstore's {@link ObjectDataType}
 *
 * @since 4.0.0
 * @author <a href="mailto:iskp.me@gmail.com">Elve.Xu</a>
 */
class CropDataType extends ObjectDataType {
  ...
  /**
   * For very common values (e.g. 0 and 1) we save space by encoding the value in the tag. e.g.
   * TAG_BOOLEAN_TRUE and TAG_FLOAT_0.
   */
  ...
  public static Object deserialize(byte[] data) {
    try (ByteArrayInputStream in = new ByteArrayInputStream(data)) {
      try (CropObjectInputStream is = new CropObjectInputStream(in)) {
   ...
   *
   * <p>This method interprets bytes as unsigned.
   *
  ...
  private AutoDetectDataType newType(int typeId) {
    ...
    throw DataUtils.newIllegalStateException(
        DataUtils.ERROR_INTERNAL, "Unsupported type {0}", typeId);
  }
      ...
      switch (tag) {
        ...
        default:
          if (tag >= TAG_INTEGER_0_15 && tag <= TAG_INTEGER_0_15 + 15) {
            ...
            throw DataUtils.newIllegalStateException(
                DataUtils.ERROR_FILE_CORRUPT, "Unknown tag {0}", tag);
          }
      }
    }
  ...
  abstract static class AutoDetectDataType implements DataType {
    ...
    /** The Base. */
    protected final CropDataType base;
    /** The Type id. */
     ...
     */
    AutoDetectDataType(CropDataType base, int typeId) {
      this.base = base;
      this.typeId = typeId;
    }
  ...
  static class CharacterType extends AutoDetectDataType {
     ...
     * @param base the base
     */
    CharacterType(CropDataType base) {
  ...
  static class FloatType extends AutoDetectDataType {
    ...
    public void write(WriteBuffer buff, Object obj) {
      ...
      if (f == CropDataType.FLOAT_ZERO_BITS) {
        buff.put((byte) TAG_FLOAT_0);
      } else if (f == CropDataType.FLOAT_ONE_BITS) {
  ...
  static class DoubleType extends AutoDetectDataType {
    ...
    public void write(WriteBuffer buff, Object obj) {
      ...
      if (d == CropDataType.DOUBLE_ZERO_BITS) {
        buff.put((byte) TAG_DOUBLE_0);
      } else if (d == CropDataType.DOUBLE_ONE_BITS) {
  ...
  static class BigIntegerType extends AutoDetectDataType {
     ...
     * @param base the base
     */
    BigIntegerType(CropDataType base) {
  ...
  static class BigDecimalType extends AutoDetectDataType {
     ...
     * @param base the base
     */
    BigDecimalType(CropDataType base) {
  ...
  static class ObjectArrayType extends AutoDetectDataType {
    ...
    private final CropDataType elementType = new CropDataType();
     ...
     * @param base the base
     */
    ObjectArrayType(CropDataType base) {
    ...
    public Object read(ByteBuffer buff, int tag) {
      ...
      if (ct == -1) {
        ...
        try {
          ...
          throw DataUtils.newIllegalStateException(
              DataUtils.ERROR_SERIALIZATION, "Could not get class {0}", componentType, e);
        }
      } else {
  ...
  static class SerializedObjectType extends AutoDetectDataType {
     ...
     * @param base the base
     */
    SerializedObjectType(CropDataType base) {
    ...
    //                if
    // (cce.getMessage().contains("xyz.vopen.framework.cropdb.store.compat.v3.Compat$IndexType")) {
    //                    return null;
    //                }
    //            } else if (iae.getCause() != null && iae.getCause() instanceof
