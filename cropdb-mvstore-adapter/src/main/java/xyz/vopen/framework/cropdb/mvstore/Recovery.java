...
/**
 * The crop database recovery utility.
 *
...
public class Recovery {
  /**
   * The block size (physical sector size) of the disk. The store header is written twice, one copy
   * in each block, to ensure it survives a crash.
   */
  ...
  /**
   * Attempt a database file recovery by rolling back to the newest good version. Writes the log
  ...
  public static boolean recover(String fileName, PrintWriter writer) {
    ValidationUtils.notNull(fileName, "fileName cannot be null");
    ValidationUtils.notEmpty(fileName, "fileName cannot be empty");
    ValidationUtils.notNull(writer, "writer cannot be null");
  ...
  private static boolean repair(String fileName, PrintWriter pw) {
    ...
    try (OutputStream ignore =
      ...
      while (version >= 0) {
        pw.println(
            version == Long.MAX_VALUE ? "Trying latest version" : ("Trying version " + version));
        pw.flush();
      ...
      throw new CropIOException("Failed to repair database", e);
    }
    return repaired;
  }
  ...
  private static Chunk readChunkHeader(ByteBuffer buff, long start) {
    ...
    try {
      ...
      throw DataUtils.newIllegalStateException(
          DataUtils.ERROR_FILE_CORRUPT, "File corrupt reading chunk at position {0}", start, e);
    }
    throw DataUtils.newIllegalStateException(
        DataUtils.ERROR_FILE_CORRUPT, "File corrupt reading chunk at position {0}", start);
  }
    ...
    try (MVStore store = new MVStore.Builder().fileName(fileName).readOnly().open()) {
      ...
      for (Map.Entry<Integer, Chunk> e : chunks.entrySet()) {
        ...
        pw.printf(
            ...
            c.id, formatTimestamp(created, fileCreated), getPercent(c.maxLenLive, c.maxLen), c.len);
        if (c.maxLenLive == 0) {
